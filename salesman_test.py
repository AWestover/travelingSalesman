...
def calc_dist(some_pts):
    ...
    the_best_length = path_dist(path, pt_dists)
    print(the_best_length)
    return the_best_length

dists = []
num_trials = 100
for k in range(0, num_trials):
    points = gen_pts(num_towns, 30.0)
    dists.append(calc_dist(points))
print("average ", np.average(dists))

import travelingSalesmanSetup as tsSetup


def calc_dist2(points):
    pt_dists = gen_distances(points)
    path = [0]
    for i in range(0, num_towns-1):
        path.append(tsSetup.smallest_positive(pt_dists[path[-1]], path))
    path.append(0)
    path = tsSetup.switcher(path, num_towns, pt_dists)
    print(tsSetup.path_dist(path, pt_dists))
    return tsSetup.path_dist(path, pt_dists)

print("COMMENCING TRIAL 2 RESULTS SHOULD BE LARGER")
dists2 = []
for k in range(0, num_trials):
    points = gen_pts(num_towns, 30.0)
    dists2.append(calc_dist2(points))
print("average ", np.average(dists2))
